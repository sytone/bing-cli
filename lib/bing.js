...
const imageToAscii = require('./simple-ascii.js');
...
var bingSearchCLI = function() {
  ...
  function parseParams() {
   ...
   var queryType = 'search';
     if (Object.prototype.hasOwnProperty.call(queryPath, (params[0] || '').toLowerCase())) {
        queryType = params[0] || '';
  ...
  var qs = Object.entries(params.qs)
    .map(([k, v]) => v ? `${k}=${v}` : '')
    .filter(x => x)
    .join('&');
  var url = null;
  ...
  function parseColor(color) {
    ...
    try {
      ...
      // Ignore parsing errors - don't need the error value
    }
    return rgb;
  }
  ...
  function ImageDisplayHandlerAscii(url, name, retryHandler, altUrl, cb) {
    ...
    */
    ...
    // Use our simple ASCII converter
    const asciiOptions = {
      width: Math.round(80 * (_params.image.scale || 1)),
      chars: _params.image.bw ? ' ██████████████' : ' .,:;i1tfLCG08@'
    };

    imageToAscii(url, asciiOptions, 
      (err, converted) => { 
        if (err) {
          if (retryHandler) {
  ...
  function ImageCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      let contentUrl = info.value.map(item => item.contentUrl)[0] || '';
      contentUrl = resolveUrl(contentUrl);
      let url = info.value.map(item => item.thumbnailUrl)[0];
      let name = info.value.map(item => item.name)[0];
      if (contentUrl && url) {
        ...
        renderFunction(url1, name, retryHandler, url2, function(_err) {
        });
      }
    }
  }
  ...
  function ImagesCallback(error, response, body) {
    if (!error && response.statusCode == 200) {    
      ...
      let urls = info.value.map(item => item.thumbnailUrl);
      let names = info.value.map(item => item.name);
      urls.forEach(function(url, i) {
        ...
        opt.fallback = function() {
           ...
           imageToAscii(url, asciiOptions, (err, ascii) => {
             if (err) {
               console.log(`Error converting image to ASCII: ${err.message}`);
             } else {
               console.log(ascii);
             }
           });
        };
        imgcat(url, opt).then(function() { var msg = names[i]; console.log(`${i+1}: ${msg}`.bold); console.log(); });;
     });
    }
  }
  ...
  function SearchCallback(error, response, body) {
    if (handleResponse(error, response)) {
      ...
      info.forEach(function(record, i) {
  ...
  function SuggestCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      info.suggestionGroups.forEach(function(record, _i) {
         let name = record.name;
         if (name === 'Web') {
           record.searchSuggestions.forEach(function(line, i) {
  ...
  function EntitiesCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (entities.value && entities.value.length > 0) {
         ...
         entities.value.forEach(function(line, i) {
  ...
  function EntityCallbackPrimary(error, response, body) {
    ...
    if (!error && response.statusCode == 200) {
      ...
      var entities = info.entities || {};
      if (entities.value && entities.value.length > 0) {
         entities.value.forEach(function(line, _i) {
          const name = line.name;
          const url = line.url;
          const description = line.description || '';
          const epi = line.entityPresentationInfo || {};
          ...
          const isMatching = _params.place ? 
            categories.some(cat => placeCategories.includes(cat)) : true;
          if (isMatching && count < 1) {
            ...
            if (imageUrl) {
              var renderFunction = _params.image.ascii ? ImageDisplayHandlerAscii : ImageDisplayHandlerInline;
              renderFunction(imageUrl, imageName, null, null, function(_err) {
              });    
            }
            count++;
          }
  ...
  function RelatedCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (relatedSearches.value && relatedSearches.value.length > 0) {
         console.log();
         console.log("Related Searches".inverse);
         relatedSearches.value.forEach(function(line, i) {
           console.log(`${i+1}: ${line.displayText}`.bold);
         })
      }
      console.log();
  ...
  function NewsCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      info.value.forEach(function(record, i) {
         let dt = new Date(record.datePublished);
         let provider = (record.provider || []).map(p => p.name)[0] || "";
  ...
  axios({
    ...
    method: 'GET',
    responseType: 'text'  // Keep response as text since callbacks expect to parse JSON
  })
  .then(response => {
    // Simulate request-like response object
    const mockResponse = {
      statusCode: response.status,
      headers: response.headers
    };
    params.callback(null, mockResponse, response.data);
  })
  .catch(error => {
